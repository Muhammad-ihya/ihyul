...
# Judul Aplikasi
st.title("Kalkulator Kebutuhan Gula dalam Buah")

# Menu Navigasi
menu = st.sidebar.selectbox("Pilih Menu", ["Penjelasan Gula", "Kalkulator Kebutuhan Gula"])

# Penjelasan tentang Gula
if menu == "Penjelasan Gula":
    st.header("Apa itu Gula dalam Buah?")
    st.write("""
        Gula adalah jenis karbohidrat sederhana yang memberikan rasa manis pada makanan dan minuman. 
        Dalam buah, gula hadir dalam bentuk alami seperti fruktosa dan glukosa. 
        Meskipun gula dalam buah memberikan energi, mereka juga mengandung serat, vitamin, dan mineral yang penting bagi kesehatan.

        Beberapa manfaat gula alami dari buah:
        - Memberikan energi cepat.
        - Menyediakan nutrisi penting.
        - Memiliki serat yang membantu pencernaan.

        Namun, penting untuk mengonsumsi gula dalam jumlah yang tepat, karena konsumsi berlebihan dapat menyebabkan masalah kesehatan.
    """)

# Kalkulator Kebutuhan Gula
elif menu == "Kalkulator Kebutuhan Gula":
    # Data gula per 100 gram buah
    buah_data = {
        'Nama Buah': ['Apel', 'Arbei', 'Apricot', 'Anggur', 'Alpukat', 'Bit' , 'Belimbing','Bengkuang',
                      'Blueberi','Blewah','Ceri','Ciplukan','Carica','Cermai','Cranberry','Cempedak',
                      'Delima','Durian','Duku','Jeruk','Jambu Biji','Jambu Air','Kurma', 'Kedondong',
                      'Kelapa','Kecapi','Kelengkeng','Kiwi','Kesemek','Leci','Labu','Lemon',
                      'Mangga','Murbei','Matoa','Mengkudu', 'Manggis','Melon','Markisa',
                      'Naga','Nangka','Nanas','Pepaya','Pir','Persik','Plum',
                      'Pisang','Rambutan','Sirsak','Sukun','Salak',
                      'Stroberi','Semangka','Sawo','Tin',
                      'Tomat','Tebu','Timun','Zaitun'],
        'Gula (gram)': [10,8,9,16,0.7,8,5,1.8,14,8,8,3,3,3,
                        4,13.5,16,19,20,9,
                        9,4.5,66,6,3,
                        14,65,9,20,
                        21.5,3.5,2.5,
    ...
    def hitung_kebutuhan_gula(usia , berat_badan):
        kebutuhan_kalori = berat_badan * 24  
        kebutuhan_gula = (kebutuhan_kalori * 0.1) / 4  
    ...
    if buah_terpilih:
        ...
        def kesimpulan(usia , berat_badan , total_gula , kebutuhan_gula):
            ...
            kesimpulan_teks = (
                f"Dengan kategori {usia_kategori} dan berat badan {berat_badan} kg , "
                f"kebutuhan gula harian Anda adalah sekitar {kebutuhan_gula:.2f} gram.\n"
...
# Tambahkan background dengan overlay hitam (opacity 0.7)
add_gradient_overlay_background(image_url=image_url, overlay_opacity=0.7)
